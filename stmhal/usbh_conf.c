...
/*******************************************************************************
                       HCD BSP Routines - appears to be same
STM32Cube_FW_F4_V1.8.0\Projects\STM324xG_EVAL\Applications\USB_Host\DualCore_Standalone\Src
STM32Cube_FW_F4_V1.8.0\Projects\STM324xG_EVAL\Applications\USB_Host\MSC_Standalone\Src
*******************************************************************************/
/**
...
void HAL_HCD_MspInit(HCD_HandleTypeDef *hhcd)
{
  ...
  if(hhcd->Instance == USB_OTG_FS)
  {
    ...
    //Expect init for PB4-BLU-PS
    //GPIO_InitStruct.Pin = GPIO_PIN_5; //TODO remove
    /*GPIO_InitStruct.Pin = GPIO_PIN_2;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    //HAL_GPIO_Init(GPIOH, &GPIO_InitStruct);  //TODO remove
  ...
  {
   /**On STM32F405 USB OTG HS Core will operate in Full speed mode
    *
    *EMBEDDED Physical interface
    * USB_OTG_HS FS GPIO Configuration
    * PB14 ------> USB_OTG_HS_DM
    * PB15 ------> USB_OTG_HS_DP */
   ...
   tprintf("~UsbHsMspinit~");
    ...
    /*  VBUS Pin - setup for checking power */
...
void HAL_HCD_MspDeInit(HCD_HandleTypeDef *hhcd)
{
  ...
  {  
    ...
    PB15     ------> USB_OTG_HS_DP 
    */
    HAL_GPIO_DeInit(GPIOB, GPIO_PIN_14|GPIO_PIN_15);//TODO check if should do this

    /* Peripheral interrupt Deinit*/
     HAL_NVIC_DisableIRQ(OTG_HS_IRQn);//TODO check if should do this
...
{
  mp_hal_stdout_tx_str("~Connect~");//TODO remove
...
USBH_StatusTypeDef USBH_LL_Init (USBH_HandleTypeDef *phost)
{
  ...
  else /*phost->id == USB_OTG_HS_CORE_ID*/
  {
    /*Set LL Driver parameters */
    hhcd_HS.Instance = USB_OTG_HS;
    hhcd_HS.Init.Host_channels = 11;
    hhcd_HS.Init.dma_enable = 0;   //DMA set later
    hhcd_HS.Init.low_power_enable = 0;
    hhcd_HS.Init.phy_itface = HCD_PHY_EMBEDDED; // !HCD_PHY_ULPI - seems dup with USB_OTG_EMBEDDED_PHY
    hhcd_HS.Init.Sof_enable = 0;
    hhcd_HS.Init.speed = HCD_SPEED_HIGH;  // expected HCD_SPEED_FULL but demo uses HCD_SPEED_HIGH
    hhcd_HS.Init.vbus_sensing_enable = ENABLE;
    hhcd_HS.Init.use_external_vbus = DISABLE;  //demo uses '1'
    /* Link The driver to the stack */
    hhcd_HS.pData = phost;
    phost->pData = &hhcd_HS;
    /*Initialize LL Driver */
    if (HAL_HCD_Init(&hhcd_HS) != HAL_OK)
    {
  	  tprintf ("usbh_LL HS Fail \n");
      return USBH_FAIL;
    } else 	{tprintf ("usbh_LL HS init");}

    USBH_LL_SetTimer (phost, HAL_HCD_GetCurrentFrame(&hhcd_HS));
    pyb_usbHS_checkStatus("usbhS");
  }
