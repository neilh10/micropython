...
/*----------------------------------------------*/
/* Put a character to phy device                */
/*----------------------------------------------*/
void xput_mp (char c) {
	uart_tx_strn(MP_STATE_PORT(pyb_stdio_uart), &c, 1);
}
/*----------------------------------------------*/
/* Put a character                              */
/*----------------------------------------------*/
...
/*----------------------------------------------*/
/* Put out time & null-terminated string                 */
/*----------------------------------------------*/

static void xputhdr (	/* Put a time leader */
) {
...
#define RTC_PRE_MAX 0xff
#define RTC_PRE_SHFT 2
	xprintf("\r\n%02d%02d%02d-%02x:",time.Hours,time.Minutes,time.Seconds,
			(unsigned int)(RTC_PRE_MAX-time.SubSeconds) //make digital fraction of 1/256 second
			);
}

void tputs (					/* Put a string to the default device */
	const char* str				/* Pointer to the string */
)
{
	xputhdr();
...
void tprintf (			/* Put a formatted string to the default device */
	const char*	fmt,	/* Pointer to the format string */
	...					/* Optional arguments */
)
{
